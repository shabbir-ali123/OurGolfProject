...
const BookedTeachers: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      try {
        ...
        if (response.data && response.data.bookedAppointments && response.data.bookedAppointments.length > 0) {
  ...
  return (
    <div className="grid grid-cols-11 gap-0 mx-0 md:mx-16 lg:mx-16 xl:mx-8 ">
      ...
      <div className="col-span-12 p-4 md:col-span-12 lg:col-span-3 xl:col-span-3 lg:overflow-y-auto scrollbar lg:max-h-screen ">
        ...
        <SingleTeacherBooked openModal={openModal} handleBookAppointment={handleBookAppointment} showTeacherDetails={showTeacherDetails} />
  ...
  {selectedTeacher !== null && <TeacherConDetail teacherDetails={selectedTeacher} />}
</div>
      {showModal && (
