export const ScoreSlider = ({item}:any) => {
 ...
 const nickName = item?.userScoreCard.nickName;
  const displayNickName = nickName && nickName.length > 6 ? `${nickName.substring(0, 6)}` : nickName;
  return (
      ...
      <div className="relative z-1 bg-blue">
        <div className="flex gap-6">
          <div className="p-1">
            ...
            <h4 className="m-0 mb-2 flex items-center gap-1"><img className="w-10 h-10 rounded-full" src={item?.userScoreCard.imageUrl} alt="" /> {displayNickName}</h4>
            <h4 className="m-0 mb-2">POS: 2/10</h4>
          </div>
          <div className="flex items-center gap-2 bg-cover rounded-lg"  style={{
            ...
            <div className="px-1">
              ...
              <div>
                ...
                <ul className="flex gap-2  p-0 m-0 py-2 text-white">
                  <li className="list-none text-[10px] tracking-wide" ><span className="bg-white text-black  px-1">PAR</span>  4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4</li> 
                </ul>
                <ul className="flex gap-2  p-0 m-0  px-1">
                  {/* {arr.map((i:any)=>(
                    <li className="list-none" >{i}</li>
                  ))} */}
                                      <li className="list-none text-[10px] tracking-wide text-white" >{scorePerShot}</li>
