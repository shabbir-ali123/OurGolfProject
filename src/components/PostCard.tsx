...
const PostCard = () => {
  const { handleDeletePost, post } = postContext();
  ...
  return (
    <div className="relative grid grid-cols-1 md:grid-cols-2 gap-4 bg-white">
      {post.map((post: Post) => {
        ...
        return (
          <Link to={`/read-post/${post.id}`} key={post.id}>
            <div className="flex p-4 relative z-1 rounded-lg" style={{ boxShadow: "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px" }}>
              <img
              ...
              <div className="p-4">
                    ...
                    {activeDropdownPostId === post.id && (
                      <div className="absolute right-[20px] top-0 w-[100px] overflow-hidden bg-white">
                        <ul className="p-0 m-0">
                          ...
                          <li className="list-none p-2 hover:shadow-lg text-start">
                            <a className="decoration-none text-[#43bcb0] hover:text-[red]" onClick={() => deletePost(post.id)}>Delete</a>
                          </li>
                        </ul>
                      </div>
                    )}
                ...
                <div className="mt-2">
                  ...
                  <div className="flex mt-6 space-x-4">
                    <span className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer" onClick={handleInteraction} data-interaction="comment">
                      <EnvelopeIcon className="w-4 h-4 cursor-pointer text-[#00D1FF]" aria-hidden="true" />
                    ...
                    <div className="flex items-center gap-0">
                      ...
                      {post.PostLikes.filter((like: any) => like.counter).length} Likes
                    </div>
                    <span className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer" onClick={handleInteraction} data-interaction="share">
                      <ShareIcon className="w-4 h-4 cursor-pointer" aria-hidden="true" data-interaction="share" />
                      Share
                    </span>
                  </div>
                </div>
